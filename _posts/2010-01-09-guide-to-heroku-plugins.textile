...
When adding new commands into the heroku gem, if you're going to add whole set of features like @bundles@ or @addons@ then create your own class to do it.  If you're only adding one simple command like the switch plugin, then just add it to @Heroku::Command::Base@.  This is what my switch command looks like:
